...
def render_one(parts: list[Part], filename: str, title: str, elev: float, azim: float, annotated: bool = False) -> None:
    ...
    if annotated:
        callouts = {
            "Piso vivienda": ("CASA · 344 × 260 × 254 mm", 110, -105),
            "Base invernadero": ("INVERNADERO · 200 × 280 × 225 mm", -80, -95),
            "Plataforma porche": ("PORCHE · 200 × 156 × 154 mm", 5, 95),
            "Torre Jarvis cuerpo": ("JARVIS · 84 × 94 × 302 mm", 105, -30),
            "Gabinete técnico base": ("GABINETE · 88 × 120 × 269 mm", 135, 55),
...
def write_svg_v3() -> None:
    ...
    r(25,230,250,350,"#dff4ea","INVERNADERO","200 × 280 × 225")
    r(300,300,430,325,"#f3dfbf","VIVIENDA ABIERTA","344 × 260 · techo removible")
    r(340,72,250,195,"#dbeafe","PORCHE","200 × 156 × 154")
    r(750,245,105,118,"#d6e4ff","JARVIS","84 × 94 × 302")
    r(870,235,110,150,"#e6edf3","GABINETE","88 × 120 × 269")
    ...
    for x, y, w, h in [(25,65,92,92),(870,235,110,150),(355,380,145,105),(535,380,145,105)]:
        sx, sy, sw, sh = ox + x*sc, oy + (650-y-h)*sc, w*sc, h*sc
        svg.append(f'<rect x="{sx}" y="{sy}" width="{sw}" height="{sh}" fill="none" stroke="#9b51e0" stroke-width="3" stroke-dasharray="10 6"/>')
    svg += [
        f'<line x1="{ox}" y1="{oy+650*sc+44}" x2="{ox+1000*sc}" y2="{oy+650*sc+44}" class="dim"/><text x="{ox+500*sc}" y="{oy+650*sc+35}" text-anchor="middle" class="l" fill="#155eef">800</text>',
...
def write_pdf_v3() -> Path:
    ...
    _pdf_dim(c,ox,oy-8*mm,ox+1000*s,oy-8*mm,'800');_pdf_dim(c,ox+300*s,oy+650*s+7*mm,ox+730*s,oy+650*s+7*mm,'344 VIVIENDA')
    c.saveState();c.translate(ox-8*mm,oy);c.rotate(90);_pdf_dim(c,0,0,650*s,0,'520');c.restoreState()
    ...
    notes=['Invernadero 200 × 280 × 225','Casa 344 × 260 × 254 total','Porche 200 × 156 × 154','Jarvis 84 × 94 × 302','Gabinete 88 × 120 × 269','Depósito real 92 × 92 × 100']
    ...
    _pdf_dim(c,20*mm+300*.34*mm,126*mm,20*mm+730*.34*mm,126*mm,'344');_pdf_dim(c,20*mm+750*.34*mm,120*mm,20*mm+855*.34*mm,120*mm,'84')
    ...
    cols=[('BASE','800×520×12','1'),('CASA','344×260','1 set'),('INVERNADERO','200×280×225','1'),('PORCHE','200×156×154','1'),('JARVIS','84×94×302','1'),('GABINETE','88×120×269','1')]
...
def validate_model(parts: list[Part]) -> dict:
    ...
    expected = {
        ...
        "Piso vivienda": (344, 260, 7),
        "Base invernadero": (200, 280, 7),
        "Plataforma porche": (200, 156, 8),
        "Torre Jarvis cuerpo": (84, 94.4, 290),
        "Gabinete técnico base": (88, 120, 7),
    ...
    report = {
        ...
        "criterio": "Huella X/Y compactada; alturas y huellas del hardware real conservadas.",
